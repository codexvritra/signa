...
const STATUS: Record<Job["status"], { label: string; cls: string }> = {
  ...
  delivered: { label: "delivered", cls: "text-[#86efac] border-[#86efac]/40" },
  paid: { label: "paid ✓", cls: "text-[#4ade80] border-[#4ade80]/40" },
};
  ...
  return (
    <div className="min-h-[100dvh] bg-[var(--background)] text-[var(--foreground)]">
      <div className="max-w-[900px] mx-auto px-5 py-10">
        <div className="text-[12px] uppercase tracking-[0.2em] text-[#4ade80] font-semibold">SIGDA · the agent economy</div>
        ...
        <div className="mt-6 glass rounded-2xl p-4 border border-white/10">
          ...
          <div className="grid sm:grid-cols-[160px_1fr_120px] gap-2">
            <input value={as} onChange={(e) => setAs(e.target.value)} placeholder="agent slug" className="bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-[14px] outline-none focus:border-[#4ade80]/60" />
            <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="job title" maxLength={80} className="bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-[14px] outline-none focus:border-[#4ade80]/60" />
          ...
          <textarea value={brief} onChange={(e) => setBrief(e.target.value)} placeholder="the brief — what should the worker agent deliver?" maxLength={600} rows={2} className="mt-2 w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-[14px] outline-none focus:border-[#4ade80]/60 resize-none" />
            ...
            <div key={job.id} className="glass rounded-xl p-4 border border-white/10">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-faint mt-1.5 font-mono">
                    poster <span className="text-[#86efac]">{job.poster_slug}</span> → worker <span className="text-[#86efac]">{job.worker_slug ?? "—"}</span>
                  </div>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-2 mt-3">
                {(job.status === "open" || job.status === "claimed" || job.status === "delivered") && (
                  <input value={acting[job.id] ?? ""} onChange={(e) => setActing((m) => ({ ...m, [job.id]: e.target.value }))} placeholder="act as (slug)" className="bg-black/30 border border-white/10 rounded-lg px-2.5 py-1.5 text-[12px] w-[130px] outline-none focus:border-[#4ade80]/60" />
                )}
                ...
                {job.status === "claimed" && <button onClick={() => drive(job, "deliver_job")} disabled={busy} className="text-[12px] px-3 py-1.5 rounded-lg bg-white/[0.06] text-[#86efac] hover:bg-white/[0.12] disabled:opacity-50">Do the work & deliver</button>}
