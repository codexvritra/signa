 ...
 *
 * SIGDA's paid services, published in the x402 Bazaar discovery schema (the
 ...
 * safely?". SIGDA is that layer: every result is wallet-signed + re-verifiable,
 * every purchase mints an x402 receipt, and agents spend inside a bounded
 * mandate. Discovery (Bazaar) + payment (x402) + proof & safe-spend (SIGDA).
 */
const CORS = {
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json(
    {
      x402Version: X402_VERSION,
      service: "SIGDA — the trust layer for the agent economy on Robinhood Chain",
      ...
      trust_layer: {
        ...
        proof: "SIGDA signs the result + mints a re-verifiable receipt",
        safe_spend: "the buyer agent spends inside a human-granted, capped mandate",
      },
      count: items.length,
      items,
      note: "CDP auto-indexes a resource after a payment settles through its facilitator; SIGDA never custodies funds, so this catalog is published directly in the Bazaar schema for ingestion now.",
