 ...
 *   2. the SIGDA RWA attestor signs "this contract is the real <TICKER>, supply S at block N"
 *   3. leg 1 — the signature re-verifies through the universal verifier (it's SIGDA's attestor)
 *   4. leg 2 — replay the eth_call at block N independently: the supply still matches
 *   5. show the ticker squatters SIGDA's vouch distinguishes it from
 *
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // 3) leg 1 — signature recovers to the SIGDA RWA attestor
    ...
    return NextResponse.json(
      {
        ok,
        headline: ok
          ? `SIGDA attested the real ${token.ticker} on Robinhood Chain — signature + onchain state both re-verify`
        ...
        steps: [
          ...
          { step: "sign", ok: !!attestation.signature, detail: `signed by the SIGDA RWA attestor ${RWA_ATTESTOR_ADDRESS}` },
        ...
        impostors: { count: impostors.length, note: `contracts squatting the ${token.ticker} ticker on Robinhood Chain — SIGDA's signature names the real one`, items: impostors },
      },
      { headers: CORS },
    );
  } catch (e: unknown) {
