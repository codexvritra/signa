...
export default function SigdaDirectoryPage() {
  ...
  return (
    <div className="min-h-[100dvh] bg-[var(--background)] text-[var(--foreground)]">
      <div className="max-w-[680px] mx-auto px-5 py-12 sm:py-16">
        <div className="text-[12px] uppercase tracking-[0.2em] text-[#4ade80] font-semibold">SIGDA Mail · the directory</div>
        ...
        <p className="text-[15px] text-muted mt-2 max-w-[560px] leading-relaxed">
          Every name here is a wallet inbox on Robinhood Chain — <span className="text-white">{loaded ? handles.length : "…"}</span> claimed. Each is wallet-signed and re-verified, so a name always points to the wallet that proved it. DM anyone by their <span className="text-[#86efac]">@sigda</span> name.
        </p>

        <div className="mt-6 flex gap-2">
          <input value={q} onChange={(e) => setQ(e.target.value.toLowerCase())} placeholder="search names" className="flex-1 bg-black/30 border border-white/10 rounded-lg px-3 py-2.5 text-[14px] outline-none focus:border-[#4ade80]/60" />
        ...
        <div className="mt-6 flex flex-col gap-2">
          ...
          {filtered.map((h) => (
            <a
              key={h.handle}
              href={`/sigda/${h.handle}`}
              className="glass rounded-xl px-4 py-3.5 border border-white/[0.06] hover:bg-white/[0.04] hover:border-[#4ade80]/30 transition-colors flex items-center gap-3"
            >
              ...
              <div className="min-w-0">
                <div className="text-[15px] font-semibold text-[#86efac] truncate">{h.handle}@sigda</div>
