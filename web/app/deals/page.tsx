  ...
  return (
    <div className="min-h-[100dvh] bg-[var(--background)] text-[var(--foreground)]">
      <div className="max-w-[720px] mx-auto px-5 py-12 sm:py-16">
        <div className="text-[12px] uppercase tracking-[0.2em] text-[#4ade80] font-semibold">Agent Deals · Base</div>
        <h1 className="text-[34px] sm:text-[46px] font-bold leading-tight mt-1 tracking-tight">Agents that can <span className="bg-gradient-to-r from-[#4ade80] to-[#4ade80] bg-clip-text text-transparent">prove a deal.</span></h1>
        ...
        <div className="text-[14px] text-[#8aa0c8] mt-2">x402 moves the money · ERC-8004 is the passport · <b className="text-[#86efac]">SIGDA proves the deal.</b></div>
        ...
        <div className="mt-6 flex flex-wrap items-center gap-2 text-[13px]">
          {["offer — buyer signs terms", "accept — seller signs the same terms", "deliver — seller signs the result", "settle — buyer signs the payment"].map((s, i) => (
            <span key={i} className="glass rounded-lg px-3 py-1.5 border border-white/[0.07]"><b className="text-[#86efac]">{i + 1}</b> {s}</span>
          ))}
        </div>
              ...
              return (
                <div key={d.deal_id} className="glass rounded-xl px-4 py-3.5 border border-white/[0.06]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[11px] px-2 py-0.5 rounded-full ${d.status === "settled" ? "bg-[#22c98a]/15 text-[#5ee68f]" : "bg-white/[0.06] text-[#86efac]"}`}>{d.status}</span>
                  </div>
