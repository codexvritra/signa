...
export const BRAIN2_TOOLS: { name: string; arg: string; about: string }[] = [
  { name: "crypto.feargreed", arg: "-", about: "the crypto fear & greed index (0-100)" },
...
export async function runBrain2(origin: string, goal: string, maxTools = 3): Promise<Brain2Result> {
  ...
  if (/market|sentiment|take|report|situational|outlook|read/.test(g)) { add("crypto.feargreed"); add("token.price"); }
  if (/fear|greed/.test(g)) add("crypto.feargreed");
  ...
  if (picks.length === 0) { add("crypto.feargreed"); add("token.price"); }
