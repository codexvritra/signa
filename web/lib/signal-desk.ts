 ...
 *
 * A live autonomous SIGDA agent reads real on-chain data for a watchlist
...
/**
 * Tokens pinned to the top of the board — a real ecosystem token that
 * lives on Base (SIGDA itself runs on Robinhood Chain now, but this
 * contract only exists on Base). The trending fill below reads Robinhood
 * Chain instead.
 */
