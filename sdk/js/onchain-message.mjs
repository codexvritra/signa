 ...
 * calldata. You don't need us to send or read one — you need a wallet, gas,
 * and an RPC.
 *
 * Requires viem (`npm i viem`).
 *
 *   # send (costs a little ETH for gas)
 *   PK=0xYOUR_PRIVATE_KEY \
 *   node onchain-message.mjs send 0xRecipientAddress "gm, this lives on Robinhood Chain forever"
 *
 ...
 *   RPC=https://your-robinhood-rpc node onchain-message.mjs read 0x…
 */
import { createWalletClient, createPublicClient, http, toHex, hexToString, defineChain } from "viem";
...
async function send(to, body) {
  ...
  console.log("\nit's on Robinhood Chain now. read it back with:\n  node onchain-message.mjs read " + hash);
}
